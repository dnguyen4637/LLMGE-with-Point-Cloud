...
(this might not work since we do not know where we got the dataset, Dr. Zutty acquired the dataset and it is stored inside a shared storage folder)

Also necessary changes needed to run the LLMGE on Pointnet++ is to change all paths to your own paths in your directory (all paths point to my absolute directory). 
