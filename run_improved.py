...
def generate_template(PROB_EOT, GEN_COUNT, TOP_N_GENES, SOTA_ROOT, SEED_NETWORK, ROOT_DIR):
    ...
    if (PROB_EOT > np.random.uniform()) and (GEN_COUNT > 0):
        ...
        parts_x = split_file(f"{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{top_gene}.py")
...
def write_bash_script(input_filename_x=f'{SOTA_ROOT}/models/pointnet2_cls_ssg.py',
                      ...
                      output_filename=f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg.py',
...
def create_individual(container, temp_min=0.05, temp_max=0.4):
    ...
    successful_sub_flag, job_id, local_output = submit_bash(file_path, 
                                              ...
                                              output_filename =f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{gene_id}.py',
                                              gpu=LLM_GPU,
                                              python_file='src/llm_mutation.py', 
...
def submit_run(gene_id):
    def write_bash_script_py(gene_id, train_file='./sota/Pointnet_Pointnet2_pytorch/train_classification.py'):
        ...
        python_runline = ('PYTHONPATH=/home/hice1/dnguyen448/scratch/llm-guided-evolution/sota/Pointnet_Pointnet2_pytorch/models/llmge_models:$PYTHONPATH 'f'python {train_file} --batch_size 216 --model "pointnet2_cls_ssg_{gene_id}" {tmp}')
...
def check4model2run(gene_id):
    ...
    print(f'Checking for: SOTA_ROOT ./models/llmge_models/pointnet2_cls_ssg_{gene_id}.py')
    model_path = f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{gene_id}.py'
...
def customCrossover(ind1, ind2):
    def combine_elements(ind1, ind2, temp_min=0.05, temp_max=0.1):
        ...
        successful_sub_flag, job_id, local_output = submit_bash(file_path, 
                                          input_filename_x=f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{gene_id_1}.py',
                                          input_filename_y=f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{gene_id_2}.py',
                                          output_filename=f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{new_gene_id}.py',
                                          gpu=LLM_GPU,
...
def customMutation(individual, indpb, temp_min=0.02, temp_max=0.35):
    ...
    temperature = round(random.uniform(temp_min, temp_max), 2)
    successful_sub_flag, job_id, local_output = submit_bash(file_path, 
                                              input_filename_x= f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{old_gene_id}.py',
                                              output_filename = f'{SOTA_ROOT}/models/llmge_models/pointnet2_cls_ssg_{new_gene_id}.py',
                                              gpu=LLM_GPU,
                                              python_file='src/llm_mutation.py', 
                                              top_p=0.1, temperature=temperature)
