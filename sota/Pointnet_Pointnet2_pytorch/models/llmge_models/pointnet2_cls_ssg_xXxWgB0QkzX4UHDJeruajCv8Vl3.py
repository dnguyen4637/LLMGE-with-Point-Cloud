...
def calculate_kl_divergence(mu, logvar):
    ...
    return kld_loss / mu.shape[0]

class GetLoss(nn.Module):
    def __init__(self):
        super(GetLoss, self).__init__()

    def forward(self, pred, target, trans_feat, model):
        total_loss = F.nll_loss(pred, target)

        # Add L2 regularization loss
        l2_reg_loss = calculate_l2_regularization(model)
        total_loss += l2_reg_loss

        # Add KL divergence loss
        kl_div_loss = calculate_kl_divergence(trans_feat['mu'], trans_feat['logvar'])
