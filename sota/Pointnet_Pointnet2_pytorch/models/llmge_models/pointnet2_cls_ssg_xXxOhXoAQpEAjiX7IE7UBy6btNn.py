...
class get_loss(nn.Module):
    ...
    def forward(self, pred, target, trans_feat):
        ...
        # Consistency Regularization
        perturbed_inputs = pred + torch.randn_like(pred) * self.eps
        perturbed_outputs = self.forward(perturbed_inputs, target, trans_feat)
        consistency_loss = self.consistency_criterion(F.log_softmax(perturbed_outputs / self.eps, dim=1),
                                                      F.softmax(perturbed_outputs / self.eps, dim=1))
