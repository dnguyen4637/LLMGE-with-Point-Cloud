...
class get_loss(nn.Module):
    ...
    def forward(self, pred, target, trans_feat):
        ...
        attn_similarity = self.similarity_fn(trans_feat, attn_pred)
        attn_loss = self.attention_loss(attn_similarity, attn_target)

        # Consistency Regularization
        perturbed_inputs = self.transform(trans_feat)
        perturbed_outputs = self(pred, target, perturbed_inputs)
        perturbed_outputs = torch.clamp(perturbed_outputs, min=self.eps, max=1 - self.eps)
        consistency_loss = F.kl_div(F.log_softmax(pred, dim=1), perturbed_outputs, reduction='sum') / batch_size
